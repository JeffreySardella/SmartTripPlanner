    ...
    ILogger<AgentService> logger) : IAgentService
{
    ...
    private async Task<object> ExecuteToolAsync(LlmToolCall toolCall, int? currentRunTripId, Action<int> setTripId)
    {
        ...
        return toolCall.Function.Name switch
        {
            "get_calendar_view" => await calendarService.GetCalendarViewAsync(
                DateTime.Parse(args["start"].ToString()!, CultureInfo.InvariantCulture, DateTimeStyles.RoundtripKind),
                DateTime.Parse(args["end"].ToString()!, CultureInfo.InvariantCulture, DateTimeStyles.RoundtripKind)),
            ...
            "validate_travel" => travelService.ValidateTravel(
                ...
                DateTime.Parse(args["departure_time"].ToString()!, CultureInfo.InvariantCulture, DateTimeStyles.RoundtripKind),
                DateTime.Parse(args["arrival_deadline"].ToString()!, CultureInfo.InvariantCulture, DateTimeStyles.RoundtripKind)),
    ...
    private async Task<object> AddTripEventWithPersistence(Dictionary<string, object> args, int? currentRunTripId, Action<int> setTripId)
    {
        ...
        var start = DateTime.Parse(args["start"].ToString()!, CultureInfo.InvariantCulture, DateTimeStyles.RoundtripKind);
        var end = DateTime.Parse(args["end"].ToString()!, CultureInfo.InvariantCulture, DateTimeStyles.RoundtripKind);
